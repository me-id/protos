// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: did/did_v1/did.proto

package did_v1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type KeyType int32

const (
	KeyType_KEYTYPE_UNSPECIFIED KeyType = 0
	KeyType_MULTIKEY            KeyType = 1
	KeyType_P256KEY2024         KeyType = 2
	KeyType_MLDSA87KEY2024      KeyType = 3
	KeyType_MLKEM1024KEY2024    KeyType = 4
)

// Enum value maps for KeyType.
var (
	KeyType_name = map[int32]string{
		0: "KEYTYPE_UNSPECIFIED",
		1: "MULTIKEY",
		2: "P256KEY2024",
		3: "MLDSA87KEY2024",
		4: "MLKEM1024KEY2024",
	}
	KeyType_value = map[string]int32{
		"KEYTYPE_UNSPECIFIED": 0,
		"MULTIKEY":            1,
		"P256KEY2024":         2,
		"MLDSA87KEY2024":      3,
		"MLKEM1024KEY2024":    4,
	}
)

func (x KeyType) Enum() *KeyType {
	p := new(KeyType)
	*p = x
	return p
}

func (x KeyType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (KeyType) Descriptor() protoreflect.EnumDescriptor {
	return file_did_did_v1_did_proto_enumTypes[0].Descriptor()
}

func (KeyType) Type() protoreflect.EnumType {
	return &file_did_did_v1_did_proto_enumTypes[0]
}

func (x KeyType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use KeyType.Descriptor instead.
func (KeyType) EnumDescriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{0}
}

type Algorithm int32

const (
	Algorithm_ALG_UNSPECIFIED Algorithm = 0
	Algorithm_ED25519         Algorithm = 1 // EdDSA over Curve25519
	Algorithm_X25519          Algorithm = 2 // X25519 key agreement (was missing before)
	Algorithm_ES256           Algorithm = 3 // P-256 / secp256r1
	Algorithm_SECP256K1       Algorithm = 4 // Bitcoin/Ethereum curve
	Algorithm_ML_DSA_87       Algorithm = 5 // Dilithium-level signature
	Algorithm_ML_KEM_1024     Algorithm = 6 // Kyber KEM 1024
)

// Enum value maps for Algorithm.
var (
	Algorithm_name = map[int32]string{
		0: "ALG_UNSPECIFIED",
		1: "ED25519",
		2: "X25519",
		3: "ES256",
		4: "SECP256K1",
		5: "ML_DSA_87",
		6: "ML_KEM_1024",
	}
	Algorithm_value = map[string]int32{
		"ALG_UNSPECIFIED": 0,
		"ED25519":         1,
		"X25519":          2,
		"ES256":           3,
		"SECP256K1":       4,
		"ML_DSA_87":       5,
		"ML_KEM_1024":     6,
	}
)

func (x Algorithm) Enum() *Algorithm {
	p := new(Algorithm)
	*p = x
	return p
}

func (x Algorithm) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Algorithm) Descriptor() protoreflect.EnumDescriptor {
	return file_did_did_v1_did_proto_enumTypes[1].Descriptor()
}

func (Algorithm) Type() protoreflect.EnumType {
	return &file_did_did_v1_did_proto_enumTypes[1]
}

func (x Algorithm) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Algorithm.Descriptor instead.
func (Algorithm) EnumDescriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{1}
}

type DomainVerificationType int32

const (
	DomainVerificationType_DVT_UNSPECIFIED  DomainVerificationType = 0
	DomainVerificationType_DNS_TXT          DomainVerificationType = 1
	DomainVerificationType_HTTPS_WELL_KNOWN DomainVerificationType = 2
)

// Enum value maps for DomainVerificationType.
var (
	DomainVerificationType_name = map[int32]string{
		0: "DVT_UNSPECIFIED",
		1: "DNS_TXT",
		2: "HTTPS_WELL_KNOWN",
	}
	DomainVerificationType_value = map[string]int32{
		"DVT_UNSPECIFIED":  0,
		"DNS_TXT":          1,
		"HTTPS_WELL_KNOWN": 2,
	}
)

func (x DomainVerificationType) Enum() *DomainVerificationType {
	p := new(DomainVerificationType)
	*p = x
	return p
}

func (x DomainVerificationType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (DomainVerificationType) Descriptor() protoreflect.EnumDescriptor {
	return file_did_did_v1_did_proto_enumTypes[2].Descriptor()
}

func (DomainVerificationType) Type() protoreflect.EnumType {
	return &file_did_did_v1_did_proto_enumTypes[2]
}

func (x DomainVerificationType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use DomainVerificationType.Descriptor instead.
func (DomainVerificationType) EnumDescriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{2}
}

type VMKey struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Controller    string                 `protobuf:"bytes,2,opt,name=controller,proto3" json:"controller,omitempty"`
	Type          KeyType                `protobuf:"varint,3,opt,name=type,proto3,enum=didme.KeyType" json:"type,omitempty"`
	Alg           Algorithm              `protobuf:"varint,4,opt,name=alg,proto3,enum=didme.Algorithm" json:"alg,omitempty"`
	Pk            string                 `protobuf:"bytes,5,opt,name=pk,proto3" json:"pk,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *VMKey) Reset() {
	*x = VMKey{}
	mi := &file_did_did_v1_did_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *VMKey) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VMKey) ProtoMessage() {}

func (x *VMKey) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VMKey.ProtoReflect.Descriptor instead.
func (*VMKey) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{0}
}

func (x *VMKey) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *VMKey) GetController() string {
	if x != nil {
		return x.Controller
	}
	return ""
}

func (x *VMKey) GetType() KeyType {
	if x != nil {
		return x.Type
	}
	return KeyType_KEYTYPE_UNSPECIFIED
}

func (x *VMKey) GetAlg() Algorithm {
	if x != nil {
		return x.Alg
	}
	return Algorithm_ALG_UNSPECIFIED
}

func (x *VMKey) GetPk() string {
	if x != nil {
		return x.Pk
	}
	return ""
}

type Service struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Type          string                 `protobuf:"bytes,2,opt,name=type,proto3" json:"type,omitempty"`
	Version       string                 `protobuf:"bytes,3,opt,name=version,proto3" json:"version,omitempty"`
	Endpoint      []byte                 `protobuf:"bytes,4,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Service) Reset() {
	*x = Service{}
	mi := &file_did_did_v1_did_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Service) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Service) ProtoMessage() {}

func (x *Service) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Service.ProtoReflect.Descriptor instead.
func (*Service) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{1}
}

func (x *Service) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Service) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *Service) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *Service) GetEndpoint() []byte {
	if x != nil {
		return x.Endpoint
	}
	return nil
}

type Attestation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Alg           Algorithm              `protobuf:"varint,1,opt,name=alg,proto3,enum=didme.Algorithm" json:"alg,omitempty"`
	Vm            string                 `protobuf:"bytes,2,opt,name=vm,proto3" json:"vm,omitempty"`
	Sig           []byte                 `protobuf:"bytes,3,opt,name=sig,proto3" json:"sig,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Attestation) Reset() {
	*x = Attestation{}
	mi := &file_did_did_v1_did_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Attestation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Attestation) ProtoMessage() {}

func (x *Attestation) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Attestation.ProtoReflect.Descriptor instead.
func (*Attestation) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{2}
}

func (x *Attestation) GetAlg() Algorithm {
	if x != nil {
		return x.Alg
	}
	return Algorithm_ALG_UNSPECIFIED
}

func (x *Attestation) GetVm() string {
	if x != nil {
		return x.Vm
	}
	return ""
}

func (x *Attestation) GetSig() []byte {
	if x != nil {
		return x.Sig
	}
	return nil
}

type Proof struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Type          string                 `protobuf:"bytes,2,opt,name=type,proto3" json:"type,omitempty"`
	Cryptosuite   string                 `protobuf:"bytes,3,opt,name=cryptosuite,proto3" json:"cryptosuite,omitempty"`
	Purpose       string                 `protobuf:"bytes,4,opt,name=purpose,proto3" json:"purpose,omitempty"`
	Vm            string                 `protobuf:"bytes,5,opt,name=vm,proto3" json:"vm,omitempty"`
	Created       string                 `protobuf:"bytes,6,opt,name=created,proto3" json:"created,omitempty"`
	Jws           string                 `protobuf:"bytes,7,opt,name=jws,proto3" json:"jws,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Proof) Reset() {
	*x = Proof{}
	mi := &file_did_did_v1_did_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Proof) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Proof) ProtoMessage() {}

func (x *Proof) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Proof.ProtoReflect.Descriptor instead.
func (*Proof) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{3}
}

func (x *Proof) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Proof) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *Proof) GetCryptosuite() string {
	if x != nil {
		return x.Cryptosuite
	}
	return ""
}

func (x *Proof) GetPurpose() string {
	if x != nil {
		return x.Purpose
	}
	return ""
}

func (x *Proof) GetVm() string {
	if x != nil {
		return x.Vm
	}
	return ""
}

func (x *Proof) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *Proof) GetJws() string {
	if x != nil {
		return x.Jws
	}
	return ""
}

type DomainVerification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	T             DomainVerificationType `protobuf:"varint,1,opt,name=t,proto3,enum=didme.DomainVerificationType" json:"t,omitempty"`
	Domain        string                 `protobuf:"bytes,2,opt,name=domain,proto3" json:"domain,omitempty"`
	Method        string                 `protobuf:"bytes,3,opt,name=method,proto3" json:"method,omitempty"`
	Proof         []byte                 `protobuf:"bytes,4,opt,name=proof,proto3" json:"proof,omitempty"`
	ProofUrl      string                 `protobuf:"bytes,5,opt,name=proof_url,json=proofUrl,proto3" json:"proof_url,omitempty"`
	VerifiedAt    string                 `protobuf:"bytes,6,opt,name=verified_at,json=verifiedAt,proto3" json:"verified_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DomainVerification) Reset() {
	*x = DomainVerification{}
	mi := &file_did_did_v1_did_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DomainVerification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DomainVerification) ProtoMessage() {}

func (x *DomainVerification) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DomainVerification.ProtoReflect.Descriptor instead.
func (*DomainVerification) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{4}
}

func (x *DomainVerification) GetT() DomainVerificationType {
	if x != nil {
		return x.T
	}
	return DomainVerificationType_DVT_UNSPECIFIED
}

func (x *DomainVerification) GetDomain() string {
	if x != nil {
		return x.Domain
	}
	return ""
}

func (x *DomainVerification) GetMethod() string {
	if x != nil {
		return x.Method
	}
	return ""
}

func (x *DomainVerification) GetProof() []byte {
	if x != nil {
		return x.Proof
	}
	return nil
}

func (x *DomainVerification) GetProofUrl() string {
	if x != nil {
		return x.ProofUrl
	}
	return ""
}

func (x *DomainVerification) GetVerifiedAt() string {
	if x != nil {
		return x.VerifiedAt
	}
	return ""
}

type UpdatePolicy struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Allowed       []string               `protobuf:"bytes,1,rep,name=allowed,proto3" json:"allowed,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdatePolicy) Reset() {
	*x = UpdatePolicy{}
	mi := &file_did_did_v1_did_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdatePolicy) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdatePolicy) ProtoMessage() {}

func (x *UpdatePolicy) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdatePolicy.ProtoReflect.Descriptor instead.
func (*UpdatePolicy) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{5}
}

func (x *UpdatePolicy) GetAllowed() []string {
	if x != nil {
		return x.Allowed
	}
	return nil
}

type DIDDocument struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Core identity
	Id         string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Controller string `protobuf:"bytes,2,opt,name=controller,proto3" json:"controller,omitempty"`
	// @context
	Ctx []string `protobuf:"bytes,3,rep,name=ctx,proto3" json:"ctx,omitempty"`
	// Metadata
	Also        []string `protobuf:"bytes,4,rep,name=also,proto3" json:"also,omitempty"`
	Biometric   bool     `protobuf:"varint,5,opt,name=biometric,proto3" json:"biometric,omitempty"`
	Hardware    bool     `protobuf:"varint,6,opt,name=hardware,proto3" json:"hardware,omitempty"`
	DeviceModel string   `protobuf:"bytes,7,opt,name=device_model,json=deviceModel,proto3" json:"device_model,omitempty"`
	// Core state
	Seq        uint64   `protobuf:"varint,8,opt,name=seq,proto3" json:"seq,omitempty"`
	Prev       string   `protobuf:"bytes,9,opt,name=prev,proto3" json:"prev,omitempty"`
	Core       string   `protobuf:"bytes,10,opt,name=core,proto3" json:"core,omitempty"`
	CoreCbor   []byte   `protobuf:"bytes,11,opt,name=core_cbor,json=coreCbor,proto3" json:"core_cbor,omitempty"`
	KeyHistory []string `protobuf:"bytes,12,rep,name=key_history,json=keyHistory,proto3" json:"key_history,omitempty"`
	// Verification relationships
	Vm     []*VMKey `protobuf:"bytes,13,rep,name=vm,proto3" json:"vm,omitempty"`
	Authn  []string `protobuf:"bytes,14,rep,name=authn,proto3" json:"authn,omitempty"`
	Assert []string `protobuf:"bytes,15,rep,name=assert,proto3" json:"assert,omitempty"`
	Inv    []string `protobuf:"bytes,16,rep,name=inv,proto3" json:"inv,omitempty"`
	Ka     []string `protobuf:"bytes,17,rep,name=ka,proto3" json:"ka,omitempty"`
	// Services
	Svc []*Service `protobuf:"bytes,18,rep,name=svc,proto3" json:"svc,omitempty"`
	// Policy & attestations
	Policy *UpdatePolicy  `protobuf:"bytes,19,opt,name=policy,proto3" json:"policy,omitempty"`
	Att    []*Attestation `protobuf:"bytes,20,rep,name=att,proto3" json:"att,omitempty"`
	// Optional P-256 DataIntegrityProof
	Proof *Proof `protobuf:"bytes,21,opt,name=proof,proto3" json:"proof,omitempty"`
	// Domain verification objects
	Dv            []*DomainVerification `protobuf:"bytes,22,rep,name=dv,proto3" json:"dv,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DIDDocument) Reset() {
	*x = DIDDocument{}
	mi := &file_did_did_v1_did_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DIDDocument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DIDDocument) ProtoMessage() {}

func (x *DIDDocument) ProtoReflect() protoreflect.Message {
	mi := &file_did_did_v1_did_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DIDDocument.ProtoReflect.Descriptor instead.
func (*DIDDocument) Descriptor() ([]byte, []int) {
	return file_did_did_v1_did_proto_rawDescGZIP(), []int{6}
}

func (x *DIDDocument) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *DIDDocument) GetController() string {
	if x != nil {
		return x.Controller
	}
	return ""
}

func (x *DIDDocument) GetCtx() []string {
	if x != nil {
		return x.Ctx
	}
	return nil
}

func (x *DIDDocument) GetAlso() []string {
	if x != nil {
		return x.Also
	}
	return nil
}

func (x *DIDDocument) GetBiometric() bool {
	if x != nil {
		return x.Biometric
	}
	return false
}

func (x *DIDDocument) GetHardware() bool {
	if x != nil {
		return x.Hardware
	}
	return false
}

func (x *DIDDocument) GetDeviceModel() string {
	if x != nil {
		return x.DeviceModel
	}
	return ""
}

func (x *DIDDocument) GetSeq() uint64 {
	if x != nil {
		return x.Seq
	}
	return 0
}

func (x *DIDDocument) GetPrev() string {
	if x != nil {
		return x.Prev
	}
	return ""
}

func (x *DIDDocument) GetCore() string {
	if x != nil {
		return x.Core
	}
	return ""
}

func (x *DIDDocument) GetCoreCbor() []byte {
	if x != nil {
		return x.CoreCbor
	}
	return nil
}

func (x *DIDDocument) GetKeyHistory() []string {
	if x != nil {
		return x.KeyHistory
	}
	return nil
}

func (x *DIDDocument) GetVm() []*VMKey {
	if x != nil {
		return x.Vm
	}
	return nil
}

func (x *DIDDocument) GetAuthn() []string {
	if x != nil {
		return x.Authn
	}
	return nil
}

func (x *DIDDocument) GetAssert() []string {
	if x != nil {
		return x.Assert
	}
	return nil
}

func (x *DIDDocument) GetInv() []string {
	if x != nil {
		return x.Inv
	}
	return nil
}

func (x *DIDDocument) GetKa() []string {
	if x != nil {
		return x.Ka
	}
	return nil
}

func (x *DIDDocument) GetSvc() []*Service {
	if x != nil {
		return x.Svc
	}
	return nil
}

func (x *DIDDocument) GetPolicy() *UpdatePolicy {
	if x != nil {
		return x.Policy
	}
	return nil
}

func (x *DIDDocument) GetAtt() []*Attestation {
	if x != nil {
		return x.Att
	}
	return nil
}

func (x *DIDDocument) GetProof() *Proof {
	if x != nil {
		return x.Proof
	}
	return nil
}

func (x *DIDDocument) GetDv() []*DomainVerification {
	if x != nil {
		return x.Dv
	}
	return nil
}

var File_did_did_v1_did_proto protoreflect.FileDescriptor

const file_did_did_v1_did_proto_rawDesc = "" +
	"\n" +
	"\x14did/did_v1/did.proto\x12\x05didme\"\x8f\x01\n" +
	"\x05VMKey\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x1e\n" +
	"\n" +
	"controller\x18\x02 \x01(\tR\n" +
	"controller\x12\"\n" +
	"\x04type\x18\x03 \x01(\x0e2\x0e.didme.KeyTypeR\x04type\x12\"\n" +
	"\x03alg\x18\x04 \x01(\x0e2\x10.didme.AlgorithmR\x03alg\x12\x0e\n" +
	"\x02pk\x18\x05 \x01(\tR\x02pk\"c\n" +
	"\aService\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04type\x18\x02 \x01(\tR\x04type\x12\x18\n" +
	"\aversion\x18\x03 \x01(\tR\aversion\x12\x1a\n" +
	"\bendpoint\x18\x04 \x01(\fR\bendpoint\"S\n" +
	"\vAttestation\x12\"\n" +
	"\x03alg\x18\x01 \x01(\x0e2\x10.didme.AlgorithmR\x03alg\x12\x0e\n" +
	"\x02vm\x18\x02 \x01(\tR\x02vm\x12\x10\n" +
	"\x03sig\x18\x03 \x01(\fR\x03sig\"\xa3\x01\n" +
	"\x05Proof\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04type\x18\x02 \x01(\tR\x04type\x12 \n" +
	"\vcryptosuite\x18\x03 \x01(\tR\vcryptosuite\x12\x18\n" +
	"\apurpose\x18\x04 \x01(\tR\apurpose\x12\x0e\n" +
	"\x02vm\x18\x05 \x01(\tR\x02vm\x12\x18\n" +
	"\acreated\x18\x06 \x01(\tR\acreated\x12\x10\n" +
	"\x03jws\x18\a \x01(\tR\x03jws\"\xc5\x01\n" +
	"\x12DomainVerification\x12+\n" +
	"\x01t\x18\x01 \x01(\x0e2\x1d.didme.DomainVerificationTypeR\x01t\x12\x16\n" +
	"\x06domain\x18\x02 \x01(\tR\x06domain\x12\x16\n" +
	"\x06method\x18\x03 \x01(\tR\x06method\x12\x14\n" +
	"\x05proof\x18\x04 \x01(\fR\x05proof\x12\x1b\n" +
	"\tproof_url\x18\x05 \x01(\tR\bproofUrl\x12\x1f\n" +
	"\vverified_at\x18\x06 \x01(\tR\n" +
	"verifiedAt\"(\n" +
	"\fUpdatePolicy\x12\x18\n" +
	"\aallowed\x18\x01 \x03(\tR\aallowed\"\xea\x04\n" +
	"\vDIDDocument\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x1e\n" +
	"\n" +
	"controller\x18\x02 \x01(\tR\n" +
	"controller\x12\x10\n" +
	"\x03ctx\x18\x03 \x03(\tR\x03ctx\x12\x12\n" +
	"\x04also\x18\x04 \x03(\tR\x04also\x12\x1c\n" +
	"\tbiometric\x18\x05 \x01(\bR\tbiometric\x12\x1a\n" +
	"\bhardware\x18\x06 \x01(\bR\bhardware\x12!\n" +
	"\fdevice_model\x18\a \x01(\tR\vdeviceModel\x12\x10\n" +
	"\x03seq\x18\b \x01(\x04R\x03seq\x12\x12\n" +
	"\x04prev\x18\t \x01(\tR\x04prev\x12\x12\n" +
	"\x04core\x18\n" +
	" \x01(\tR\x04core\x12\x1b\n" +
	"\tcore_cbor\x18\v \x01(\fR\bcoreCbor\x12\x1f\n" +
	"\vkey_history\x18\f \x03(\tR\n" +
	"keyHistory\x12\x1c\n" +
	"\x02vm\x18\r \x03(\v2\f.didme.VMKeyR\x02vm\x12\x14\n" +
	"\x05authn\x18\x0e \x03(\tR\x05authn\x12\x16\n" +
	"\x06assert\x18\x0f \x03(\tR\x06assert\x12\x10\n" +
	"\x03inv\x18\x10 \x03(\tR\x03inv\x12\x0e\n" +
	"\x02ka\x18\x11 \x03(\tR\x02ka\x12 \n" +
	"\x03svc\x18\x12 \x03(\v2\x0e.didme.ServiceR\x03svc\x12+\n" +
	"\x06policy\x18\x13 \x01(\v2\x13.didme.UpdatePolicyR\x06policy\x12$\n" +
	"\x03att\x18\x14 \x03(\v2\x12.didme.AttestationR\x03att\x12\"\n" +
	"\x05proof\x18\x15 \x01(\v2\f.didme.ProofR\x05proof\x12)\n" +
	"\x02dv\x18\x16 \x03(\v2\x19.didme.DomainVerificationR\x02dv*k\n" +
	"\aKeyType\x12\x17\n" +
	"\x13KEYTYPE_UNSPECIFIED\x10\x00\x12\f\n" +
	"\bMULTIKEY\x10\x01\x12\x0f\n" +
	"\vP256KEY2024\x10\x02\x12\x12\n" +
	"\x0eMLDSA87KEY2024\x10\x03\x12\x14\n" +
	"\x10MLKEM1024KEY2024\x10\x04*s\n" +
	"\tAlgorithm\x12\x13\n" +
	"\x0fALG_UNSPECIFIED\x10\x00\x12\v\n" +
	"\aED25519\x10\x01\x12\n" +
	"\n" +
	"\x06X25519\x10\x02\x12\t\n" +
	"\x05ES256\x10\x03\x12\r\n" +
	"\tSECP256K1\x10\x04\x12\r\n" +
	"\tML_DSA_87\x10\x05\x12\x0f\n" +
	"\vML_KEM_1024\x10\x06*P\n" +
	"\x16DomainVerificationType\x12\x13\n" +
	"\x0fDVT_UNSPECIFIED\x10\x00\x12\v\n" +
	"\aDNS_TXT\x10\x01\x12\x14\n" +
	"\x10HTTPS_WELL_KNOWN\x10\x02Bt\n" +
	"\tcom.didmeB\bDidProtoP\x01Z)github.com/me-id/protos/gen/go/did/did_v1\xa2\x02\x03DXX\xaa\x02\x05Didme\xca\x02\x05Didme\xe2\x02\x11Didme\\GPBMetadata\xea\x02\x05Didmeb\x06proto3"

var (
	file_did_did_v1_did_proto_rawDescOnce sync.Once
	file_did_did_v1_did_proto_rawDescData []byte
)

func file_did_did_v1_did_proto_rawDescGZIP() []byte {
	file_did_did_v1_did_proto_rawDescOnce.Do(func() {
		file_did_did_v1_did_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_did_did_v1_did_proto_rawDesc), len(file_did_did_v1_did_proto_rawDesc)))
	})
	return file_did_did_v1_did_proto_rawDescData
}

var file_did_did_v1_did_proto_enumTypes = make([]protoimpl.EnumInfo, 3)
var file_did_did_v1_did_proto_msgTypes = make([]protoimpl.MessageInfo, 7)
var file_did_did_v1_did_proto_goTypes = []any{
	(KeyType)(0),                // 0: didme.KeyType
	(Algorithm)(0),              // 1: didme.Algorithm
	(DomainVerificationType)(0), // 2: didme.DomainVerificationType
	(*VMKey)(nil),               // 3: didme.VMKey
	(*Service)(nil),             // 4: didme.Service
	(*Attestation)(nil),         // 5: didme.Attestation
	(*Proof)(nil),               // 6: didme.Proof
	(*DomainVerification)(nil),  // 7: didme.DomainVerification
	(*UpdatePolicy)(nil),        // 8: didme.UpdatePolicy
	(*DIDDocument)(nil),         // 9: didme.DIDDocument
}
var file_did_did_v1_did_proto_depIdxs = []int32{
	0,  // 0: didme.VMKey.type:type_name -> didme.KeyType
	1,  // 1: didme.VMKey.alg:type_name -> didme.Algorithm
	1,  // 2: didme.Attestation.alg:type_name -> didme.Algorithm
	2,  // 3: didme.DomainVerification.t:type_name -> didme.DomainVerificationType
	3,  // 4: didme.DIDDocument.vm:type_name -> didme.VMKey
	4,  // 5: didme.DIDDocument.svc:type_name -> didme.Service
	8,  // 6: didme.DIDDocument.policy:type_name -> didme.UpdatePolicy
	5,  // 7: didme.DIDDocument.att:type_name -> didme.Attestation
	6,  // 8: didme.DIDDocument.proof:type_name -> didme.Proof
	7,  // 9: didme.DIDDocument.dv:type_name -> didme.DomainVerification
	10, // [10:10] is the sub-list for method output_type
	10, // [10:10] is the sub-list for method input_type
	10, // [10:10] is the sub-list for extension type_name
	10, // [10:10] is the sub-list for extension extendee
	0,  // [0:10] is the sub-list for field type_name
}

func init() { file_did_did_v1_did_proto_init() }
func file_did_did_v1_did_proto_init() {
	if File_did_did_v1_did_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_did_did_v1_did_proto_rawDesc), len(file_did_did_v1_did_proto_rawDesc)),
			NumEnums:      3,
			NumMessages:   7,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_did_did_v1_did_proto_goTypes,
		DependencyIndexes: file_did_did_v1_did_proto_depIdxs,
		EnumInfos:         file_did_did_v1_did_proto_enumTypes,
		MessageInfos:      file_did_did_v1_did_proto_msgTypes,
	}.Build()
	File_did_did_v1_did_proto = out.File
	file_did_did_v1_did_proto_goTypes = nil
	file_did_did_v1_did_proto_depIdxs = nil
}
